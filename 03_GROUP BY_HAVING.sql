...
FROM EMPLOYEE e 
GROUP BY DEPT_CODE ;
-- ** GROUP BY : EMPLOYEE 테이블에서 DEPT_CODE 컬럼 값이 같은 행들끼리 묶음(그룹)
-- 	  그룹별로 그룹함수(SUM) 연산 수행
-- ++ DEPT_CODE : 묶인 그룹 이름
...
-- 부서코드 순으로 정렬
/*4*/SELECT DEPT_CODE , COUNT(*)
/*1*/FROM EMPLOYEE 
/*2*/WHERE BONUS IS NOT NULL
/*3*/GROUP BY DEPT_CODE
/*5*/ORDER BY DEPT_CODE;
--> 0인 부서 안나옴 : 해석 순서에 다름

/*3*/SELECT DEPT_CODE , COUNT(BONUS)
/*1*/FROM EMPLOYEE 
/*2*/GROUP BY DEPT_CODE
/*4*/ORDER BY DEPT_CODE;
--> 0인 부서까지 나옴 : 해석 순서에 따라 달라짐
...
--  SELECT절에 명시한 컬럼을 조회하려면 
--  컬럼 중 그룹함수가 적용되지 않은 컬럼을 
--  모두 GROUP BY절에 작성해야함.
...
-- WHERE 절 : 지정된 테이블에서 어떤 행만을 조회 결과로 삼을 것인지
--	 					조건을 지정하는 구문(테이블 내에서 특정 행만 뽑아서 쓰겠다는 조건문)
...
-- 부서 코드 오름차순으로 정렬하세요.
SELECT DEPT_CODE , MIN(SALARY), MAX(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE 
ORDER BY DEPT_CODE;
...
-- 부서코드가 D5이면서 급여가 300만원을 초과하는 사원
-- 1) 부서코드가 D5
...
-- ** 집합 연산자 사용 시 컬럼의 타입만 일치하면 연산 수행 가능함 ** --
